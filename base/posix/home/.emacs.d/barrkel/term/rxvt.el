...
(message "Loading custom rxvt.el")
...
(defvar rxvt-function-map
  (let ((map (make-sparse-keymap)))
    ...
    ;; screen on rxvt converts home to \e[1~
    (define-key map "\e[1~" [home])
    (define-key map "\e[2~" [insert])
    (define-key map "\e[3~" [delete])
    ;; PC keyboard doesn't have select, and screen converts end to \e[4~
